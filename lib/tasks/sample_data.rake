...
namespace :db do
  ...
  task :cal => :environment do
    ...
    if false
      ...
      Event.create!(:title => "DM Sales Flyer (40% RPI & 60% off TTL Order) -- 12.4% RR -- $14.1mm ",   :starts_at => make_time(2013, 5, 27, 0), :ends_at => make_time(2013, 6, 9, 0),   :all_day => true, :description => "", :studio_id => 1, :color => "blue")
      Event.create!(:title => "DM Sales Flyer (40% RPI & 50% + $200 off TTL)",  :starts_at => make_time(2013, 6, 10, 0), :ends_at => make_time(2013, 6, 23, 0),  :all_day => true, :description => "", :studio_id => 1, :color => "blue")

      Event.create!(:title => "NPI BS (40% RPI & 60% off TTL Order) -- 1.36% RR -- $750k",           :starts_at => make_time(2013, 5, 27, 0), :ends_at => make_time(2013, 6, 2, 0),   :all_day => true, :description => "", :studio_id => 1, :color => "darkgoldenrod")
      Event.create!(:title => "NPI Baby Event (50% RPI & 60% off TTL Order) -- 1.21% RR -- $636k",   :starts_at => make_time(2013, 6, 3, 0),  :ends_at => make_time(2013, 6, 9, 0),   :all_day => true, :description => "", :studio_id => 1, :color => "darkgoldenrod")

      Event.create!(:title => "SMS (50% RPI)",                                  :starts_at => make_time(2013, 6, 10, 0), :ends_at => make_time(2013, 6, 16, 0),  :all_day => true, :description => "", :studio_id => 1, :color => "green")

      Event.create!(:title => "Email Firefly Frenzy (50% RPI)",                 :starts_at => make_time(2013, 6, 10, 0), :ends_at => make_time(2013, 6, 23, 0),  :all_day => true, :description => "", :studio_id => 1, :color => "darkorchid")
      Event.create!(:title => "Email Celebrate Savings (40% off ES Scrapbook)", :starts_at => make_time(2013, 6, 24, 0), :ends_at => make_time(2013, 6, 30, 0),  :all_day => true, :description => "", :studio_id => 1, :color => "darkorchid")

      Event.create!(:title => "Postcard July 4th & Military",                   :starts_at => make_time(2013, 6, 24, 0), :ends_at => make_time(2013, 7, 3, 0),   :all_day => true, :description => "", :studio_id => 1, :color => "fuchsia")
    end
